...
| Storage | **None — nothing touches disk.** Readings you save accumulate in memory (`st.session_state`) for the current browser session only. Export writes them to a zip you download. Refreshing or closing the tab discards them. |
...
3. In **Advanced settings**, set **Python version to 3.11+**. No secrets are
   required. Optionally set the `WHISPER_MODEL_SIZE` environment variable to
   `base` if the host is low on RAM.
...
**The app is open — anyone with the URL can use it.** There is no password gate
and no secrets to configure.

> ⚠️ **Nothing is persisted.** Your readings live in memory for the current
> browser session only. Use **📥 Data export** in the sidebar to download them as a zip
...
- **No cross-session history.** Each browser session starts empty by design. If
  you need data to survive a refresh, add a backend that scopes rows to an
  authenticated user — don't reintroduce a shared local SQLite file.
